class InstallSolidQueue < ActiveRecord::Migration[8.1]
  def change
    ...
    # Only add foreign keys if tables are actually being created
    if !table_exists?(:solid_queue_blocked_executions) || !foreign_key_exists?(:solid_queue_blocked_executions, :solid_queue_jobs)
